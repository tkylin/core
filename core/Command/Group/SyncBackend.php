...
class SyncBackend extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		// Prefetch groups for the count
		$output->writeln("Prefetch groups from external backend ...");
		$p = new ProgressBar($output);
		$max = $syncService->prefetch();
	...
	private function handleUnknownGroups(OutputInterface $output, SyncService $syncService) {
		...
		if (empty($toBeDeleted)) {
			...
			$output->writeln("Following groups are no longer known with the connected backend.");

			$output->writeln("Proceeding to remove the backend groups");
